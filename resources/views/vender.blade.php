<x-app-layout>
    ...
    <div class="py-6 max-w-xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form action="{{ route('vender.acao') }}" method="POST" class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow space-y-4">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="codigo" id="codigo"
                       class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-red-500 focus:ring-red-200 dark:bg-gray-900 dark:text-white"
                       required>
            </div>
            ...
            <div>
                <button type="submit"
                        class="inline-flex items-center px-4 py-2 bg-red-600 dark:bg-red-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 dark:hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    Vender
                </button>
            </div>
        </form>
